...
function getFilteredRequestHeaders(requestHeaders) {
	...
	requestHeaders.forEach((header) => {
	    // filter header to remove
	    if (!REMOVEREQUESTHEADER.includes(header.name)) {
	        // set amazon header
	        amazonRequestHeaders.push({ name: header.name, value: header.value });
	    }
	});
	return amazonRequestHeaders;
}

function createCustomAmazonHeader() {
...
function showAddedToPlaylistNotification(playlist) {
	browser.storage.sync.get([
    	...
    	if(data.amazonMusicHelperShowNotifications) {
			browser.notifications.create('amazonMusic-Playlist-' + Date.now(), {
			  	type: "basic",
				iconUrl: data.amazonMusicCurrentSong.artwork,
				title: "Neuer Song in Playlist",
				message: `${data.amazonMusicCurrentSong.title} - ${data.amazonMusicCurrentSong.artistName} zur Playlist ${playlist.name} hinzugefügt.`,
		  	});
		}
    });
}
